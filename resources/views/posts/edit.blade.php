...
<x-app-layout>
    ...
    <div class="py-2 md:py-4">
        <div class="relative w-full mx-auto sm:px-2 lg:px-4">
            ...
            <form action="{{ route('posts.update', $post->id) }}" method="POST" enctype="multipart/form-data" class="flex flex-col md:flex-row gap-4 mt-4 justify-between align-center">
                @csrf
                @method('PUT')
                    ...
                    <div class="mx-auto max-w-6xl shadow-lg py-8 px-6 relative bg-white rounded">
                        <div class="mt-4">
                            <input name="title" id="title" type="text" class="w-full text-sm px-4 py-3 rounded outline-none border-2 border-gray-200 focus:border-blue-500"
                                required placeholder="Enter post title" value="{{ $post->title }}" />
                        </div>
                        <div class="mt-4">
                            <textarea name="body" id="editor" class="w-full h-40 text-sm px-4 py-3 rounded outline-none border-2 border-gray-200 focus:border-blue-500">{{ $post->body }}</textarea>
                        </div>
                        <div class="mt-2">
            ...
            @if (session('status') === 'post-updatated')
                <x-status-message status="success" message="{{ __('Post updated') }}" description="{{ __('Your post is updated successfully.') }}" />
            @endif
